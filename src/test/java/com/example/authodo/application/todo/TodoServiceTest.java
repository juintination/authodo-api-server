...
class TodoServiceTest {
    ...
    private Todo sampleTodo(Long id, String title, String content, TodoStatus status) {
        return Todo.create(1L, title, content).toBuilder()
            .id(id)
            .status(status)
    ...
    void create_success() {
        ...
        Todo result = todoService.create(1L, "새로운 제목", "내용");
    ...
    void get_notFound() {
        given(todoRepositoryPort.findByUserIdAndId(1L, 999L)).willReturn(Optional.empty());

        assertThatThrownBy(() -> todoService.get(1L, 999L))
            .isInstanceOf(BusinessException.class)
            .hasMessageContaining(ErrorCode.TODO_NOT_FOUND.name());

        verify(todoRepositoryPort).findByUserIdAndId(1L, 999L);
    }
    ...
    void getAll_success() {
        ...
        given(todoRepositoryPort.findAllByUserId(1L)).willReturn(todos);

        todoService.getAll(1L);

        verify(todoRepositoryPort).findAllByUserId(1L);
    }
    ...
    void update_success() {
        ...
        todoService.update(1L, 1L, "새 제목", "새 내용", TodoStatus.COMPLETED);
    ...
    void update_notFound() {
        ...
        assertThatThrownBy(() ->
            todoService.update(1L, 1L, "수정", "내용", TodoStatus.PENDING))
            .isInstanceOf(BusinessException.class)
            .hasMessageContaining(ErrorCode.TODO_NOT_FOUND.name());
    }
    ...
    void delete_notFound() {
        ...
        assertThatThrownBy(() -> todoService.delete(1L, 1L))
            .isInstanceOf(BusinessException.class)
            .hasMessageContaining(ErrorCode.TODO_NOT_FOUND.name());
    }
}
